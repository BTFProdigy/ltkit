"""EXP-12 (H3): quantizer granularity ablation on KAN.
Compare soft (tanh-bound, current 'ternary') vs q5 (5-level STE) on mnist_d64 + orth_id."""
...
OUT = os.path.join(os.path.dirname(__file__), "results", "exp12_quant_h3.json")
QUANTS = ["soft", "q5"]
SEEDS = list(range(5))


def run_one(quant, seed):
    ...
    bk = {"dims": [in_d] + hidden + [out_dim], "basis_order": 6, "quant": quant}
...
def main():
    ...
    results = []
    total = len(QUANTS) * len(SEEDS)
    i = 0
    for q in QUANTS:
        for seed in SEEDS:
            i += 1
            fv, fs, el = run_one(q, seed)
            rec = {"quant": q, "seed": seed, "final_val": fv,
                   "final_sparsity": fs, "elapsed_s": el}
            results.append(rec)
            with open(OUT, "w") as f:
                json.dump(results, f, indent=2)
            print(f"[{i}/{total}] quant={q} seed={seed} val={fv:.4f} "
                  f"sp={fs:.3f} t={el:.1f}s", flush=True)
    print()
    agg = {}
    for r in results:
        agg.setdefault(r["quant"], []).append(r["final_val"])
    for q, vals in sorted(agg.items()):
        print(f"{q:4s} mean={np.mean(vals):.4f} std={np.std(vals):.4f}")
